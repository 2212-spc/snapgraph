...
## 结论
先直接回答用户问的“为什么 / 怎么办 / 这说明什么”。这一段是用户最先看到的答案，必须给判断，不要从材料清单开头。
如果证据不足，要明确说“不确定”，但仍给出一个可验证的探索假设。
不要列 source_id、文件路径、材料编号或检索诊断；这些只能放在后面的 section。
用 1-3 个短段落回答，每段不超过 3 句。
