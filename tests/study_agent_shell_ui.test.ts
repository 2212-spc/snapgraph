...
test('App adopts StudyAgent-style workspace shell structure', () => {
  ...
  assert.match(file, /providerTruthLabel/)
  assert.match(file, /providerActionLabel/)
  assert.match(file, /providerFallbackLabel/)
  assert.match(file, /真实模型只在需要生成回答时参与/)
  assert.doesNotMatch(file, /Qwen 正在组织回答/)
...
test('RecallHome exposes a compact chat-style starter and composer surface', () => {
  ...
  assert.match(file, /class="recall-box recall-command"/)
  assert.match(file, /class="composer-input-wrap"/)
  assert.match(file, /class="composer-send-row"/)
  assert.match(file, /class="recent-question-panel"/)
