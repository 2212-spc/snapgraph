...
test('CollectView includes ingest progress and memory receipt UX', () => {
  ...
  assert.match(file, /更改/)
  assert.match(file, /receipt-title-editor/)
  assert.match(file, /updateTitle/)
  assert.match(file, /function startTitleEdit\(\)/)
  assert.match(file, /function confirmTitleEdit\(\)/)
  assert.match(file, /function cancelTitleEdit\(\)/)
  assert.match(file, /const receipt = activeReceipt\.value[\s\S]*emit\('updateTitle', receipt\.source_id, nextTitle\)/)
...
test('styles include collect receipt and ingest progress classes', () => {
  ...
  assert.match(file, /\.receipt-title-editor/)
  assert.match(file, /\.receipt-title-display/)
  assert.match(file, /\.receipt-title-actions/)
  assert.match(file, /\.receipt-edit-button/)
